...
export const usePemutihan = (): UsePemutihanReturn => {
  ...
  const fetchPemutihan = async (): Promise<Pemutihan[]> => {
    ...
    try {
      ...
      return data.map((item: any) => ({
        ...
        status: (item.status?.toLowerCase() === 'kanibal' ? 'kanibal' : 'toko'),
  ...
  const mapToPemutihan = (row: any): Pemutihan => {
    return {
      ...
      status: (row.status?.toLowerCase() === 'kanibal' ? 'kanibal' : 'toko'),
    ...
    mutationFn: async (data: Omit<Pemutihan, 'id'>) => {
      try {
        ...
        const pemutihanData = {
          ...
          status: (data.status?.toLowerCase() === 'kanibal' ? 'kanibal' : 'toko'),
